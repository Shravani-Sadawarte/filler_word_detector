...
1. **Speech → Text** using OpenAI Whisper (AI-based transcription)
2. **Text → Filler Analysis** using regex-based pattern matching
...
- 🔍 Detects four common filler words: `um`, `uh`, `like`, `you know`
- 📂 Processes **multiple audio files** in one run
...
- ⚠️ Includes **error handling** for invalid or unreadable audio files
...
3. Add the `bin` folder path to your system's **Environment Variables → PATH**
...
1. Place your `.wav`  audio files inside the `audios/` folder
2. Run the script:
